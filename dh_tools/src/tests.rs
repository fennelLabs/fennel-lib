use crate::{
    get_session_public_key, get_session_secret, get_shared_secret,
    generate_brainpool_keypair, import_brainpool_keypair,
    compute_brainpool_shared_secret, brainpool_public_key_to_compressed,
};
...
fn test_brainpool_keypair_generation() {
    ...
    assert_eq!(public_key.len(), 33, "Public key should be 33 bytes (SEC1 compressed)");
...
fn test_brainpool_keypair_import() {
    ...
    let (reimported_private, reimported_public) = import_brainpool_keypair(&private_key)
...
fn test_brainpool_ecdh_with_encryption() {
    ...
    let shared_array: [u8; 32] = shared1.as_slice().try_into().expect("Shared secret not 32 bytes");
...
fn test_brainpool_public_key_compression() {
    ...
    let compressed = brainpool_public_key_to_compressed(&public_key)
        .expect("Failed to process compressed key");
    
    assert_eq!(public_key, compressed, "Compressed key should remain unchanged");
...
fn test_brainpool_deterministic_keys() {
    ...
    let (priv1, pub1) = import_brainpool_keypair(&seed)
        .expect("Failed to import keypair 1");
    let (priv2, pub2) = import_brainpool_keypair(&seed)
        .expect("Failed to import keypair 2");
